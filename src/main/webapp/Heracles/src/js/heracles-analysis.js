// configure angular
require(['angular', 'jquery', 'bootstrap', 'heracles-d3', 'jasny', 'heracles_common', 'lodash', 'monster', 'bootstrap-select', 'ohdsi_common'],
    function (angular, $, b, HeraclesD3, j, heraclesCommon, _, monster, SelectPicker, OHDSICommon) {
    angular.element().ready(function () {
        ...
        angular.module('HeraclesAnalysis', []).controller('CohortExplorerCtrl', function ($scope, $http) {
            ...
            $scope.sendJob = function (jobs) {
                ...
                $scope.job.message = "Your job(s) , " + jobName + ", is being submitted. Please wait to receive a status update...";
                if (!$("#jobStatusModal").is(":visible")) {
                    $("#jobStatusModal").modal("show");
                }

                console.log('sending ' + $scope.selectedSourceKeys.length + ' jobs...');

                for (var j = 0; j < $scope.selectedSourceKeys.length; j++) {

                    // submit job
                    var cohortJob = {};

                    cohortJob.jobName = jobName + '_' + $scope.selectedSourceKeys[j];
                    cohortJob.sourceKey = $scope.selectedSourceKeys[j];
            ...
            $(document).ready(function () {
                ...
                if (param && param !== '') {
                    ...
                    getSources(true, function(sources) {
                        $scope.showCohort(datum, sources);
