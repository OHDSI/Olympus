		(function () {
			define(["jquery", "d3", "jnj/chart", "common", "datatables", "datatables-colvis"], function ($, d3, jnj_chart, common) {
				...
				condition_occurrence.drilldown = function (concept_id, concept_name) {
					...
					$.ajax({
						type: "GET",
						url: "http://localhost:20000/achilles/" + "data?dataSourceFolder="+ page_vm.datasource().folder + "/conditions" + "&file=" + "condition_" + concept_id + ".json",//'data/' + page_vm.datasource().folder + '/conditions/condition_' + concept_id + '.json',
						success: function (data) {
							// age at first diagnosis visualization
							var boxplot = new jnj_chart.boxplot();
							bpseries = [];
							bpdata = common.normalizeDataframe(data.AGE_AT_FIRST_DIAGNOSIS);
				...
				condition_occurrence.render = function (folder) {
					...
					$.ajax({
						type: "GET",
						url: "http://localhost:20000/achilles/" + "data?dataSourceFolder="+ folder + "&file=condition_treemap.json",//'data/' + folder + '/condition_treemap.json',
